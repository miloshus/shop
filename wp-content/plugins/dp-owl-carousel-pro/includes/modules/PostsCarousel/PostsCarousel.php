...
class DPOCP_PostsCarousel extends ET_Builder_Module {

	public $slug = 'et_pb_dp_oc';
		...
		if ( $props['use_hash_thumbnail'] === 'on' ) {
			...
			if ( $props['url_anchors'] === 'on' ) {
				foreach ( $posts['hash_thumbnail_array'] as $key => $value ) {
					$hash_output .= '<a href="#' . $module_order . '_' . $key . '" ><img class="dp_ocp_hash_image" src="' . $value . '" /></a> ';
				}
			} else {
				foreach ( $posts['hash_thumbnail_array'] as $key => $value ) {
